...
for (i_proj in 1:n_proj) {
    ...
    outfile = file.path(PROJECT_DIR,proj[i_proj],paste0(proj[i_proj],"_ranks.rnk"))
    write(t(output),ncol=ncol(output),file=outfile,sep="\t")
    
    # we save gene expression data for later use
    # 1. gene annotations
    output = rbind(colnames(gene_annot),gene_annot)
    outfile = file.path(PROJECT_DIR,proj[i_proj],paste0(proj[i_proj],"_genes.txt"))
    write(t(output),ncol=ncol(output),file=outfile,sep="\t")
    
    # 2. sample annotations
    output = cbind(subj,type,sample[,c(1,2,4,5,7,8)])
    output = rbind(colnames(output),output)
    outfile = file.path(PROJECT_DIR,proj[i_proj],paste0(proj[i_proj],"_sample_annot.txt"))
    write(t(output),ncol=ncol(output),file=outfile,sep="\t")
    
    # 3. gene expression matrix
    output = expr
    outfile = file.path(PROJECT_DIR,proj[i_proj],paste0(proj[i_proj],"_expr.txt"))
    write(t(output),ncol=ncol(output),file=outfile,sep="\t")
}
